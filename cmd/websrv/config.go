...
var fallbackPath = flag.String("fallback-path", "", "Filepath relative to targetDir which serves as fallback. Set to \"/\" to serve a SPA.")
...
var gzipCompressionLevel = flag.Int("gzip-level", gzip.BestSpeed, "The compression level used for gzip compression. See the golang gzip documentation for details. Only applies to on-the-fly compression. The in-memory-fs (when used) uses for static files always gzip.BestCompression")
var h2c = flag.Bool("h2c", false, "Enable h2c (unencrypted http2)")
var h2cPort = flag.Int("h2c-port", 443, "The adveritsed port for h2c. This is the port visiable to the end user which may differ from the app port due to reverse proxies/loadbalancers. h2c is for the app itself served via the default port")
...
var metricsNamespace = flag.String("metrics-namespace", "websrv", "Prometheus namespace for the collected metrics.")
